...
function parseNumericDateToken(token) {
  ...
  // Swap month/day if it looks like MM/DD/YYYY (US format)
  if (month > 12 && day <= 12) {
    const temp = month;
    month = day;
    day = temp;
...
export function extractDateFromLabels(text){
  ...
  netCandidates.forEach(c => {
    ...
    // Penalize suspiciously large numbers that are likely phone numbers or IDs
    // (e.g. > 100,000 and integer-like, unless explicitly labeled)
    if (c.value > 100000 && Math.abs(c.value - Math.round(c.value)) < 0.01) {
      // Check if it has a strong label
      const hasStrongLabel = /total|payable|due|balance/i.test(c.line || "");
      if (!hasStrongLabel) {
        c.score -= 100; // Heavy penalty
      }
    }
  });
  ...
  // ---- Sanity Check: "Sum of Parts" vs "Total" (Text-Only Version) ----
  // Fix for common OCR error where '₹' is read as '2' or '7' (e.g. ₹200 -> 2200)
  if (best && Number.isFinite(best.value) && best.value > 0) {
    // 1. Calculate sum of all OTHER candidates found in text
    // (exclude the chosen one, exclude huge numbers)
    const otherCandidates = netCandidates.filter(c =>
      c !== best &&
      c.value > 0 &&
      c.value < best.value && // items should be smaller than total
      c.value < 100000 // exclude noise
    );

    // If netCandidates is sparse, try using allNums from the fallback scope if available
    // But here we only have local scope. Let's rely on what we have.
    ...
    // 2. Check if chosen value is suspiciously larger than probable sum
    const sVal = String(best.value);
    if (sVal.length > String(Math.round(probableSum)).length) {
      const firstDigit = sVal[0];
      if (['2', '7', '4', '1'].includes(firstDigit)) {
        const repairedVal = parseFloat(sVal.slice(1));
        if (Number.isFinite(repairedVal)) {
          const diff = Math.abs(repairedVal - probableSum);
          if (diff < 5 || diff < probableSum * 0.2) {
            console.log(`DEBUG: Repaired text-only total from ${best.value} to ${repairedVal} based on probable sum ${probableSum}`);
            best = { ...best, value: repairedVal, raw: `${best.raw} (repaired_sum)` };
          }
        }
      }
    }
...
export function parseReceiptText(text) {
  console.log("DEBUG: parseReceiptText input:", JSON.stringify(text));
  ...
  console.log("DEBUG: parseReceiptText result:", JSON.stringify(result, null, 2));
  return result;
}
...
export function parseOcrResult({ text = "", words = null } = {}) {
  ...
  // ---- Sanity Check: "Sum of Parts" vs "Total" ----
  // Fix for common OCR error where '₹' is read as '2' or '7' (e.g. ₹200 -> 2200)
  if (chosen && Number.isFinite(chosen.value) && chosen.value > 0) {
    // 1. Calculate sum of all OTHER candidates that are likely line items
    // (exclude the chosen one, exclude huge numbers, exclude dates)
    const otherCandidates = scored.filter(c =>
      c !== chosen &&
      c.value > 0 &&
      c.value < chosen.value && // items should be smaller than total
      c.value < 100000 // exclude noise
    );

    // Simple sum strategy: sum all unique values found
    // (This is a heuristic; it might double count if same item appears twice, but we use Set to minimize)
    const uniqueValues = new Set();
    let probableSum = 0;
    otherCandidates.forEach(c => {
      if (!uniqueValues.has(c.value)) {
    ...
    // 2. Check if chosen value is suspiciously larger than probable sum (e.g. ~10x or more, or just has extra digit)
    // specifically check for leading digit '2', '7', '4', '1' which are common symbol misreads
    const sVal = String(chosen.value);
    if (sVal.length > String(Math.round(probableSum)).length) {
      const firstDigit = sVal[0];
      if (['2', '7', '4', '1'].includes(firstDigit)) {
        const repairedVal = parseFloat(sVal.slice(1));
        if (Number.isFinite(repairedVal)) {
          // If repaired value is close to probable sum (within 20% or 5 units)
          const diff = Math.abs(repairedVal - probableSum);
          if (diff < 5 || diff < probableSum * 0.2) {
            console.log(`DEBUG: Repaired total from ${chosen.value} to ${repairedVal} based on probable sum ${probableSum}`);
            chosen = { ...chosen, value: repairedVal, score: chosen.score + 10 }; // Boost score to keep it
          }
        }
      }
    }
